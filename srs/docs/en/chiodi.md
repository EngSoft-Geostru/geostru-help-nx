...
In plan the anchors are arranged on a grid with spacing **i_x**
(horizontal) and **i_y** (along the slope), either **staggered**
(diamond pattern, left) or on a **rectangular grid** (right):
...
<figcaption>Bar layout in plan: staggered and rectangular grid.</figcaption>
</figure>
